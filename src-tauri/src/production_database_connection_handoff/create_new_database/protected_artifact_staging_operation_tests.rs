...
fn pre_active_publication_owners_are_non_clone_non_copy_non_serializable_and_have_no_deref() {
    ...
    assert_sealed!(FreshnessAuthenticationKeyWrapperPublishedFirstTimeSetupOperation);
    assert_not_impl!(
...
#[test]
fn first_time_setup_freshness_authentication_key_publication_moves_only_second_wrapper_and_preserves_ownership()
 {
    let mut fixture = VerificationFixture::new_unstaged();
    let staged = stage_real_fixture(&mut fixture);
    let completed = verify_all_staged_first_time_setup_operation(staged).unwrap();
    ...
    assert_boundary(
        &database_published.machine,
        "ProtectedDatabaseKeyWrapperPublished",
    );

    let allocations =
        pending_wrappers(&database_published.pending_publication).map(|bytes| bytes.as_ptr());
    ...
    .unwrap();
    let expected_freshness_key = database_published
        .pending_publication
        .protected_freshness_authentication_key_wrapper
        .as_bytes()
        .to_vec();
    let metadata = database_published.database_metadata;
    let evidence_paths = database_published.installation_evidence_paths.clone();
    let key_paths = database_published.database_key_paths.clone();
    let freshness_paths = database_published.freshness_anchor_paths.clone();
    let authenticated_anchor_stage = fs::read(
        freshness_paths
            .staged_authenticated_freshness_anchor
            .as_path(),
    )
...
#[test]
fn first_time_setup_freshness_authentication_key_publication_maps_each_filesystem_phase_without_advancing()
 {
    use FreshnessAuthenticationKeyWrapperPublicationFilesystemError as Filesystem;
    for (filesystem, operation_error) in [
        (
            Filesystem::PrepublicationRejected,
...
#[test]
fn first_time_setup_freshness_authentication_key_publication_machine_failure_is_post_mutation_without_rollback()
 {
    let mut fixture = VerificationFixture::new_unstaged();
    let staged = stage_real_fixture(&mut fixture);
    let completed = verify_all_staged_first_time_setup_operation(staged).unwrap();
    let prepared = prepare_first_time_setup_active_publication(completed).unwrap();
    let mut database_published = publish_first_time_setup_database_key_wrapper(prepared).unwrap();
    let freshness_paths = database_published.freshness_anchor_paths.clone();
    database_published.machine = protected_artifact_staging::begin::<
        FirstTimeSetupPublicationStateMachine,
    >(&database_published.authority);
    assert_eq!(
...
#[test]
fn first_time_setup_freshness_authentication_key_publication_owner_and_boundary_are_exact_and_sealed()
 {
    let source = production();
    let fields = source
        .split_once(

...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming readability-and-integrity validation transition, consuming live metadata and SQLite-header validation transition, and consuming identity-only database/evidence correspondence transition are implemented and accepted through commit `8b880621e9d7cf9dcff30eaab31f84958926d024`. The repository now has distinct opaque keyed-but-unvalidated, readability-and-integrity-validated, `LiveMetadataAndHeaderValidatedProductionDatabaseConnection`, and `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection` owners; it still has no live freshness classification, startup authority, operational production database-opening flow, setup, schema, migration, backup, restore, recovery, replacement, or frontend flow. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains unchanged.

## 2. Authority and objective

The active objective is to preserve the approved typed trust-chain sequence after acceptance of the identity-only database/evidence correspondence adapter. That implemented boundary consumes `LiveMetadataAndHeaderValidatedProductionDatabaseConnection` plus `TrustedCurrentInstallationEvidenceAssessment` and returns only an opaque `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection` on correspondence success. The next separately scoped boundary will consume that whole owner and compose it with the existing local-anchor freshness foundation. Its exact inputs, anchor-loading boundary, live taxonomy, success ownership, close behavior, module placement, operational caller, and startup integration remain subject to separate architecture review. Schema creation, migration, setup/startup integration, backup/restore, recovery, replacement, frontend behavior, IPC, Tauri commands, and operational use remain later separate boundaries.
...
The repository is a Tauri 2 foundation with four unavailable React areas, one non-sensitive Rust health command, typed Rust storage, evidence, key, metadata, correspondence, and freshness foundations, a Windows production SQLCipher dependency, a private Windows-only raw-key application primitive, metadata-only production database-file inspection, a private guarded read-only connection handoff, a consuming readability-and-integrity validation transition, a consuming live metadata and SQLite-header validation transition, and a consuming identity-only database/evidence correspondence transition. The current chain returns opaque `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection`, retaining the same guarded connection lifetime, one owned `DatabaseMetadataContractV1`, and one owned `TrustedCurrentInstallationEvidenceAssessment`; it has no operational caller. The repository has no live freshness classification, operational production database-opening flow, schema, authentication, recovery, backup, or parish workflow.
...
The guarded read-only SQLCipher connection handoff, its consuming readability-and-integrity validation transition, the succeeding live metadata and SQLite-header validation transition, and the identity-only correspondence transition are complete and accepted. The handoff composes the successful metadata-only inspection proof, a separate connection-lifetime write guard, guard/proof identity binding, one read-only path-based SQLite/SQLCipher open, SQLite-handle/proof identity binding, connection hardening, one key application, post-key query-only enforcement, and opaque keyed-but-unvalidated ownership. The next transition runs the two fixed integrity/readability checks and returns an opaque readability-and-integrity-validated owner. The implemented live stage then strictly observes the two fixed SQLite headers and exactly one fixed metadata-row query, reuses the existing pure parser and structural validator, enforces header/metadata agreement, and returns `LiveMetadataAndHeaderValidatedProductionDatabaseConnection`. The implemented correspondence stage consumes that owner plus `TrustedCurrentInstallationEvidenceAssessment`, reuses the pure six-field classifier exactly once, and returns `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection`. All four stages preserve explicit close-failure ownership. Freshness composition with the existing local anchor is the next separately scoped boundary; schema creation and operational startup/setup authority remain later stages.

## 7. Allowed scope

Documentation-only reconciliation of the implemented and CI-accepted identity-only database/evidence correspondence boundary in `PLANS.md`, `docs/architecture.md`, `docs/product-decisions.md`, `docs/security-and-data.md`, and `docs/verification.md`. Existing connection-handoff, readability-and-integrity, live metadata/header, correspondence, installation-evidence, and freshness code and repository safeguards remain unchanged.

## 8. Prohibited scope

This documentation work authorizes no code, test, dependency, schema or migration, SQL or PRAGMA execution, database opening or VFS/path adapter, freshness design or implementation, setup/startup/recovery integration, backup/restore, replacement, Tauri command, IPC, frontend change, generated inventory, application runtime, database creation, header mutation, or destructive operation. It records accepted correspondence implementation evidence only and must not be represented as freshness, startup, operational-opening, schema, migration, recovery, backup/restore, replacement, frontend, IPC, or Tauri implementation evidence.
...
The succeeding live metadata and SQLite-header validation transition and identity-only database/evidence correspondence transition are implemented and accepted as described below. Freshness composition with the existing local anchor is the next separately scoped boundary. Startup/setup integration, schema creation, migrations, recovery, backup/restore, replacement, frontend, IPC, Tauri commands, and operational callers remain later separately approved work.
...
- Implemented and accepted production foundation: the exact Windows production `rusqlite` configuration, private raw-key application primitive, metadata-only production database-file inspection, guarded read-only connection handoff, consuming readability-and-integrity validation transition, consuming live metadata and SQLite-header validation transition, and consuming identity-only database/evidence correspondence transition. The repository retains distinct opaque keyed-but-unvalidated, readability-and-integrity-validated, live-metadata-and-header-validated, and correspondence-validated owners; none provides freshness, startup, or operational authority.
- Approved but not fully implemented: the larger bounded architecture package. Deferred implementation includes live freshness composition, metadata schema creation, portable recovery, migrations, backup/restore, setup/startup/recovery authority, replacement, destructive retention or cleanup, and release automation.
- The live metadata and SQLite-header validation and identity-only correspondence implementations are accepted. Separate later scoping remains required for freshness composition, metadata schema creation, portable recovery envelope, migrations, backup/restore, setup/startup/recovery authority, database or anchor replacement, and destructive retention or cleanup.

## 15. Validation status

The accepted earlier proofs remain recorded in history. Clean CI for commit `8b880621e9d7cf9dcff30eaab31f84958926d024` (`feat(database): validate evidence correspondence`) passed in the `Bootstrap validation` workflow run `30786193482` (run number 17), whose workflow and job conclusions were both success. Frontend formatting, lint, type-check, and 5 frontend tests passed; Rust formatting and Clippy with warnings denied passed; and the locked Rust suite reported 655 passed, 0 failed, and 1 ignored. All 10 new correspondence tests passed, with none ignored or filtered out. The single ignored test remains the unrelated pre-existing manually rooted USB controlled-host test. This automated evidence does not constitute a manual application or production runtime flow; none applies because the correspondence owner has no operational caller. Freshness and operational database opening remain unimplemented. The accepted controlled-host evidence remains unchanged; the USB row remains failed and incomplete.
...
Success is possession of opaque `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection`, which privately retains exactly the unchanged `ConnectionLifetimeOwner`, one owned `DatabaseMetadataContractV1`, and one owned `TrustedCurrentInstallationEvidenceAssessment`. It retains no separate `DatabaseMetadataCorrespondence::Corresponds` proof. The trusted assessment preserves trusted loading and authentication provenance, its structurally validated evidence, and `TrustedCurrentInstallationIdentity` without retaining paths, wrapper bytes, keys, envelope bytes, native errors, or loader state. The next separately scoped freshness boundary will consume the whole correspondence owner and compose it with the existing local-anchor freshness foundation. The exact freshness signature, anchor-loading boundary, taxonomy, owner contents, close behavior, placement, operational caller, and startup integration remain undesigned pending separate architecture review.

Correspondence ignores installation generation, recovery/replacement generation, evidence-format identity and version, database creation timestamp, and evidence creation timestamp. It establishes no equal lineage, freshness, or rollback resistance. The next separately scoped boundary will consume the whole correspondence owner and compose it with the existing local-anchor freshness foundation.

...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming validation and authorization chain, Rust-owned application-startup lifecycle, explicit first-time setup lifecycle, minimal production V1 metadata/bootstrap schema, private unwired full-integrity validation boundary, and private process-local migration-confirmation state foundation are implemented through the current repository state. The first business-schema product direction is now approved only at the conceptual documentation level and is limited to `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview`. The repository still has no physical parish/business schema, production migration-confirmation path, migration, backup, restore, recovery, replacement, authentication, parish workflow, or general writable database-operation interface. This foundation does not make the full product launch-ready. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains otherwise unchanged.

## 2. Authority and objective

The current documentation objective records the approved first business-schema product direction without implementing it. The conceptual scope is exactly `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview`; physical table/column design, SQL types and codes, constraints and indexes, a schema version, migration, Rust business types, IPC, forms, and workflows remain outside scope. The existing Rust-owned migration-confirmation state/capability foundation remains implemented as previously accepted, but no production path can establish its pending opportunity or accept confirmation. Migration remains separately gated, and no claim is made that a future business schema is schema V2.
...
This stage records the first business-schema product decisions in documentation only. The guarded read-only SQLCipher startup chain, operational activation, explicit first-time setup, encrypted database creation, minimal V1 metadata/bootstrap initialization, private unwired full-integrity validation boundary, and private migration-confirmation state foundation remain the implemented baseline. Physical business-schema design and implementation, migration, recovery, replacement, backup/restore, authentication, parish workflows, release validation, and any broader database-operation interface remain unimplemented. Startup has no setup, migration, full-integrity, repair, recovery, or reset fallthrough.
...
This stage authorizes no dependency, configuration, workflow implementation, maintenance execution, authentication, physical schema or migration, SQL or PRAGMA change, writable database access, freshness redesign, recovery integration, backup/restore, replacement, Tauri command, frontend change, generated inventory, header mutation, or destructive operation. It defines no physical table or column names, SQL types, persisted codes, indexes, constraints, foreign keys, schema version, schema V2, migration plan or SQL, Rust business type, form, or IPC. It also does not define migration eligibility ownership, a migration coordinator, authentication roles, backup implementation or acceptance, writable maintenance owner, exclusivity, migration execution or update ordering, interruption/restart execution policy, or authority for restore, recovery, rekey, replacement, or cleanup.
...
## 25. Approved first business-schema product decisions

The canonical decision record is `docs/product-decisions.md`. The exact conceptual scope is `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview`; these are domain names only and do not approve physical relational names.

- Fixed categories: Baptism, Confirmation, Wedding/Marriage, Burial/Funeral, and First Communion. There is no configurable catalog or service-administration table.
- Status: new requests begin `Pending`; the only first-version transitions are `Pending -> Scheduled`, `Pending -> Cancelled`, `Scheduled -> Completed`, and `Scheduled -> Cancelled`. No reopening, reverse transition, reactivation, or generic workflow engine is approved.
- Cancellation: one unresolved review per request; review remains separate from primary status; pending review leaves `Pending` or `Scheduled` unchanged; approval moves either to `Cancelled`; rejection leaves status unchanged; resolved reviews are retained; another request may follow rejection; and dedicated staff filtering/prioritization is required.
- Scheduling: separate occurrences; one primary occurrence for Baptism, Confirmation, Wedding/Marriage, and First Communion; separately representable Funeral and Burial occurrences with required/optional cardinality unresolved; exact same local date/time conflict in one shared parish domain; no durations, end times, interval overlap, clergy/resource/capacity, or resource-scoped conflicts.
- Location and time: occurrence-owned bounded free text with no catalog; Wedding/Marriage and First Communion require a final location; Funeral and Burial may differ; schedule facts are local date/time under `Asia/Manila`, while `MM/DD/YYYY` and 12-hour AM/PM are presentation only.
- Identity and contact: internal database key only, with no human request number; request-local snapshot limited to full name, mobile/phone, and optional email; no durable person identity or person/parishioner table.
- Timestamps: request `created_at`; review `requested_at` and resolved `resolved_at`; no inferred generic audit, history, update, deletion, or soft-delete infrastructure.
- Deferrals: person and relationship normalization plus all permanent/sacramental records remain future design. Completing a request does not create a permanent record.

Persisted category/status/disposition representation, keys, field lengths and validation, location representation and rules, time encoding, review constraints, and all other SQL/physical details remain unresolved. The existing V1 bootstrap, startup validation, setup authority, migration authorization and confirmation state, evidence/correspondence/freshness, SQLCipher requirements, Rust trust boundary, and React non-authority remain unchanged. No schema version is assigned, and no migration is approved by these decisions.

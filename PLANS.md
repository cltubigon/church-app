...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming readability-and-integrity validation transition, consuming live metadata and SQLite-header validation transition, consuming identity-only database/evidence correspondence transition, preloaded normalized database-freshness transition, internal startup authorization, operational activation, and Rust-owned application-startup lifecycle integration are implemented and accepted through commit `44d2770786d4534ef37fb58b383e5b74ab73d04c`. The repository still has no setup, schema creation, migration, backup, restore, recovery, replacement, authentication, or parish workflow. This accepted lifecycle integration does not make the full product launch-ready. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains otherwise unchanged.

## 2. Authority and objective

The active documentation objective is to record the accepted Rust-owned application-startup lifecycle that composes the approved typed trust chain. The lifecycle observes canonical installation evidence early, runs the synchronous production trust chain in one Rust-owned blocking worker, re-observes canonical evidence immediately before startup authorization, passes that actual second `installation_state::InstallationEvidence` value into authorization, and performs operational activation only after authorization succeeds. Setup, schema creation, migration, backup/restore, recovery, replacement, authentication, parish workflows, and any broader operational database interface remain separate and unimplemented.
...
The repository is a Tauri 2 foundation with four unavailable React areas, one non-sensitive Rust health command, one read-only `startup_status` command, typed Rust storage and trust-chain foundations, and an accepted Rust-owned startup and orderly-shutdown lifecycle. The application window appears immediately in a non-ready loading state while one blocking Rust worker performs the synchronous production chain. `Ready` is installed only after startup authorization is followed by operational activation into a real `OperationalProductionDatabase`. React remains non-authoritative and sees only coarse lifecycle status. The repository has no schema creation, authentication, setup, migration, recovery, backup/restore, replacement, or parish workflow.
...
The guarded read-only SQLCipher connection handoff and its consuming readability/integrity, live metadata/header, identity-only correspondence, preloaded normalized freshness, startup authorization, operational activation, and Tauri application lifecycle integration are implemented and accepted. The remaining stages are schema creation, migration, setup, recovery, replacement, backup/restore, authentication, parish workflows, release validation, and any broader database-operation interface. No retry, setup, migration, repair, recovery, or reset fallthrough is part of startup.
...
This documentation work authorizes no code, test, dependency, configuration, workflow, schema or migration, SQL or PRAGMA change, freshness redesign, setup/recovery integration, backup/restore, replacement, new Tauri command or IPC, frontend change, generated inventory, application runtime change, database creation, header mutation, or destructive operation. It records only the committed lifecycle behavior and does not authorize any broader operational database interface, installation-state change, stale source-comment cleanup, or new architecture.

## 9. Dependency approvals

The accepted Windows production dependency is exactly `rusqlite = { version = "=0.39.0", default-features = false, features = ["bundled-sqlcipher-vendored-openssl"] }` under `[target.'cfg(windows)'.dependencies]`. There is no Windows `rusqlite` development dependency and no direct `libsqlite3-sys` dependency. Metadata schema creation, migrations, key and recovery publication, backup/restore, setup/recovery authority, replacement, retention, cleanup, and broader operational database APIs each require separately scoped approval.
...
The succeeding live metadata/header, correspondence, preloaded normalized freshness, startup-authorization, operational-activation, and lifecycle transitions are implemented as described below. Schema creation, migrations, setup, recovery, replacement, backup/restore, authentication, parish workflows, broader frontend/database IPC, and final release approval remain separately scoped work.
...
- Implemented production foundation: the exact Windows production `rusqlite` configuration, private raw-key application primitive, metadata-only production database-file inspection, guarded read-only connection handoff, consuming validation and startup-authorization transitions, operational activation, and lifecycle ownership. The repository retains distinct opaque owners through `StartupAuthorizedProductionDatabaseConnection`; activation then consumes that owner into `OperationalProductionDatabase` without granting arbitrary SQL or product-workflow authority.
- Approved but not fully implemented: the larger bounded architecture package. Deferred implementation includes metadata schema creation, portable recovery, migrations, backup/restore, setup/recovery authority, replacement, destructive retention or cleanup, authentication, parish workflows, broader database operations, and release automation.
...
The accepted lifecycle manual scenarios are: A, a valid complete fixture reached `Ready`; B, active evidence with the database missing became `Unavailable`; C, recognized staging at launch became `Unavailable`; D, a state change before the final canonical observation was rejected by that final observation and did not reach `Ready`; and E, close while `Starting` entered shutdown pending/drain and did not fall through to `Ready`. Scenario D verifies the implemented second-observation veto in the exercised fixture; it is not proof against every possible TOCTOU or race condition. Manual fixture export is Windows test-only and ignored. Manual startup root/pause selection is Windows plus `debug_assertions` only and is not frontend or IPC authority.

## 17. Completed work

All prior completed history remains accepted. The production foundation includes the guarded read-only connection handoff and its consuming validation chain. The Rust-owned startup worker is now the operational caller of that fixed chain; the chain still exposes no direct frontend database command, arbitrary SQL surface, or frontend-supplied path or evidence.

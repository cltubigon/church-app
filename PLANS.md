...
The repository is a Tauri 2 foundation with four unavailable React areas, one non-sensitive Rust health command, typed Rust storage, evidence, key, metadata, correspondence, and freshness foundations, a Windows production SQLCipher dependency, a private Windows-only raw-key application primitive, metadata-only production database-file inspection, a private guarded read-only connection handoff, and a consuming readability-and-integrity validation transition. The transition returns an opaque readability-and-integrity-validated owner and has no operational caller. The repository has no live metadata/header observation, metadata-validated or operational production database-opening flow, schema, authentication, recovery, backup, or parish workflow.

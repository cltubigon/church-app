...
The active documentation objective is to lock the Rust-owned construction boundary for the already-approved migration-specific interactive confirmation. The local human operator physically using Church App under the dedicated Church App production Windows account is the maintenance-authorizing actor, but authority exists only after Rust has established an exact pending production-database-migration confirmation opportunity and the operator explicitly confirms it through the application. This is an authorization and consent boundary, not application-level identity authentication. Production database migration remains the sole initial maintenance authorization identity; no generic maintenance-operation enum, broad generic maintenance-authorization capability, or intermediate confirmed-intent capability is approved. The policy is documentation only: Rust state, commands, UI, authority types, maintenance execution, authentication, backup/restore, writable database access, migration, schema V2, and parish/business schema remain unimplemented.
...
Documentation-only recording of the approved Rust-owned migration-confirmation construction boundary, limited to `PLANS.md`, `docs/architecture.md`, `docs/product-decisions.md`, `docs/security-and-data.md`, and `docs/verification.md`. Code and repository safeguards remain unchanged.
...
No `ConfirmedMigrationIntent` abstraction is approved: it would duplicate the same trust fact without adding a separate boundary. The future confirmation transition should directly construct and internally retain `ProductionDatabaseMigrationAuthorization`. That future authorization remains private, non-`Copy`, non-`Clone`, non-serializable, non-persistent, redacted, and exactly-once consumable, but it is not implemented by this documentation task.
...
The Rust state machine may later be implemented and tested before migration execution exists, but no successful production confirmation path may exist until a separately approved Rust migration-specific coordinator or context producer can legitimately establish `Pending`. A dormant command that only returns `notPending` is not required, and neither future command is implemented here.

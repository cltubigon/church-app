...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming validation and authorization chain, Rust-owned application-startup lifecycle, explicit first-time setup lifecycle, minimal production V1 metadata/bootstrap schema, private unwired full-integrity validation boundary, and private process-local migration-confirmation state foundation are implemented through the current repository state. The repository still has no production migration-confirmation path, migration, backup, restore, recovery, replacement, authentication, parish workflow, parish/business schema, or general writable database-operation interface. This foundation does not make the full product launch-ready. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains otherwise unchanged.

## 2. Authority and objective

The active objective implements only the Rust-owned state/capability foundation for the already-approved migration-specific interactive confirmation. The local human operator physically using Church App under the dedicated Church App production Windows account remains the maintenance-authorizing actor, but no production path can establish the pending opportunity or accept confirmation. This is an authorization and consent boundary, not application-level identity authentication. Production database migration remains the sole initial maintenance authorization identity; no generic maintenance-operation enum, broad generic maintenance-authorization capability, or intermediate confirmed-intent capability is approved. Commands, UI, maintenance execution, authentication, backup/restore, writable database access, migration, schema V2, and parish/business schema remain unimplemented.
...
The guarded read-only SQLCipher startup chain, operational activation, explicit first-time setup, encrypted database creation, minimal V1 metadata/bootstrap initialization, private unwired full-integrity validation boundary, and private migration-confirmation state foundation are implemented. Migration is still the next genuinely unimplemented gated storage stage; this state foundation does not approve it. Future parish/business schema work, recovery, replacement, backup/restore, authentication, parish workflows, release validation, and any broader database-operation interface also remain unimplemented. Startup has no setup, migration, full-integrity, repair, recovery, or reset fallthrough.

## 7. Allowed scope

The private Rust-only migration-confirmation state/capability foundation, lifecycle-owned shutdown invalidation, focused tests, and minimal factual documentation updates. No production `Pending` producer or confirmation/cancellation IPC is included.

## 8. Prohibited scope

This stage authorizes no dependency, configuration, workflow, maintenance execution, authentication, new schema or migration, SQL or PRAGMA change, writable database access, freshness redesign, recovery integration, backup/restore, replacement, Tauri command, frontend change, generated inventory, header mutation, or destructive operation. It does not define migration eligibility ownership, a migration coordinator, a generic maintenance state machine or operation framework, migration source/target versions, schema V2, a complete IPC representation, UI wording or layout, authentication roles, backup implementation or acceptance, writable maintenance owner, exclusivity, migration execution or update ordering, interruption/restart execution policy, or authority for restore, recovery, rekey, replacement, or cleanup.

...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming readability-and-integrity validation transition, and consuming live metadata and SQLite-header validation transition are implemented and accepted through commit `aa2317eddeca73d7709f84136f12067d80e0a881`. The repository now has distinct opaque keyed-but-unvalidated, readability-and-integrity-validated, and `LiveMetadataAndHeaderValidatedProductionDatabaseConnection` owners; it still has no database/evidence correspondence, live freshness classification, startup authority, operational production database-opening flow, setup, schema, migration, backup, restore, recovery, replacement, or frontend flow. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains unchanged.

## 2. Authority and objective

The active objective is to preserve the approved typed trust-chain sequence while making the identity-only database/evidence correspondence adapter the next separately scoped implementation boundary. Its architecture is approved here; it is not implemented and has no passing implementation evidence. The boundary consumes `LiveMetadataAndHeaderValidatedProductionDatabaseConnection` plus `TrustedCurrentInstallationEvidenceAssessment` and can return only an opaque `DatabaseEvidenceCorrespondenceValidatedProductionDatabaseConnection` on correspondence success. Freshness, schema creation, migration, setup/startup integration, backup/restore, recovery, replacement, frontend behavior, IPC, Tauri commands, and operational use remain later separate boundaries.
...
The repository is a Tauri 2 foundation with four unavailable React areas, one non-sensitive Rust health command, typed Rust storage, evidence, key, metadata, correspondence, and freshness foundations, a Windows production SQLCipher dependency, a private Windows-only raw-key application primitive, metadata-only production database-file inspection, a private guarded read-only connection handoff, a consuming readability-and-integrity validation transition, and a consuming live metadata and SQLite-header validation transition. The current chain returns an opaque `LiveMetadataAndHeaderValidatedProductionDatabaseConnection` retaining the same guarded connection lifetime and one owned `DatabaseMetadataContractV1`; it has no operational caller. The repository has no database/evidence correspondence, live freshness classification, operational production database-opening flow, schema, authentication, recovery, backup, or parish workflow.
...
The guarded read-only SQLCipher connection handoff, its consuming readability-and-integrity validation transition, and the succeeding live metadata and SQLite-header validation transition are complete and accepted. The handoff composes the successful metadata-only inspection proof, a separate connection-lifetime write guard, guard/proof identity binding, one read-only path-based SQLite/SQLCipher open, SQLite-handle/proof identity binding, connection hardening, one key application, post-key query-only enforcement, and opaque keyed-but-unvalidated ownership. The next transition runs the two fixed integrity/readability checks and returns an opaque readability-and-integrity-validated owner. The implemented live stage then strictly observes the two fixed SQLite headers and exactly one fixed metadata-row query, reuses the existing pure parser and structural validator, enforces header/metadata agreement, and returns `LiveMetadataAndHeaderValidatedProductionDatabaseConnection`. All three stages preserve explicit close-failure ownership. The approved identity-only database/evidence correspondence adapter is the next separately scoped implementation boundary; freshness, schema creation, and operational startup/setup authority remain later stages.

## 7. Allowed scope

Documentation-only recording of the approved identity-only database/evidence correspondence architecture in `PLANS.md`, `docs/architecture.md`, `docs/product-decisions.md`, `docs/security-and-data.md`, and `docs/verification.md`. Existing connection-handoff, readability-and-integrity, live metadata/header, installation-evidence, pure correspondence, and freshness code and repository safeguards remain unchanged.

## 8. Prohibited scope

This documentation work authorizes no code, test, dependency, schema or migration, SQL or PRAGMA execution, database opening or VFS/path adapter, correspondence implementation, freshness design or implementation, setup/startup/recovery integration, backup/restore, replacement, Tauri command, IPC, frontend change, generated inventory, application runtime, database creation, header mutation, or destructive operation. It records an approved future architecture only and must not be represented as implementation or passing evidence.
...
- [x] Approve and canonically document, without implementing, the identity-only database/evidence correspondence adapter.
...
The succeeding live metadata and SQLite-header validation transition is implemented and accepted as described below. The identity-only database/evidence correspondence architecture is approved as the next separately scoped implementation boundary and remains unimplemented. Freshness, startup/setup integration, schema creation, migrations, recovery, backup/restore, replacement, frontend, IPC, Tauri commands, and operational callers remain later separately approved work.
...
- Implemented and accepted production foundation: the exact Windows production `rusqlite` configuration, private raw-key application primitive, metadata-only production database-file inspection, guarded read-only connection handoff, consuming readability-and-integrity validation transition, and consuming live metadata and SQLite-header validation transition. The repository retains distinct opaque keyed-but-unvalidated, readability-and-integrity-validated, and live-metadata-and-header-validated owners; none provides startup or operational authority.
- Approved but not fully implemented: the larger bounded architecture package. Deferred implementation includes database/evidence correspondence, live freshness classification, metadata schema creation, portable recovery, migrations, backup/restore, setup/startup/recovery authority, replacement, destructive retention or cleanup, and release automation.
- The live metadata and SQLite-header validation implementation is accepted. Separate later scoping remains required for database/evidence correspondence, freshness composition, metadata schema creation, portable recovery envelope, migrations, backup/restore, setup/startup/recovery authority, database or anchor replacement, and destructive retention or cleanup.

## 15. Validation status

The accepted earlier proofs remain recorded in history. Clean CI for commit `aa2317eddeca73d7709f84136f12067d80e0a881` (`feat(database): validate live metadata and headers`) passed in the `Bootstrap validation` workflow run `30778837736`, whose workflow and job conclusions were both success. Frontend formatting, lint, type-check, and tests passed; Rust formatting and Clippy with warnings denied passed; and the locked Rust suite reported 645 passed, 0 failed, and 1 ignored. All 15 new live metadata/header tests passed, with no new test ignored or filtered out. The single ignored test remains the unrelated pre-existing manually rooted USB controlled-host test. This automated evidence does not constitute a manual application or production runtime flow; none applies because the live owner has no operational caller. Database/evidence correspondence, freshness, and operational database opening remain unimplemented. The accepted controlled-host evidence remains unchanged; the USB row remains failed and incomplete.
...
The readability-and-integrity-validated owner can now be consumed by the fixed live metadata and SQLite-header transition. It observes `application_id`, then `user_version`, then the explicit 12-column `LIMIT 2` metadata query; applies exact result-shape, storage-class, UTF-8, parsing, structural-validation, and header/metadata agreement rules; and returns only `LiveMetadataAndHeaderValidatedProductionDatabaseConnection`. That owner retains the unchanged connection/guard/inspection lifetime unit and one owned `DatabaseMetadataContractV1`. Its failures and consuming close paths preserve the accepted explicit-close ownership discipline and redaction boundary.
...
## 19. Approved next implementation boundary: identity-only correspondence

The approved future consuming transition is:

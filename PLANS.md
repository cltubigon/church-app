...
Active multi-stage initiative. Carlo has explicitly approved the bounded production database and evidence-correspondence architecture package, including the corrected `ApplicationDatabaseFormatIdentity` exactly-16-byte SQLite `BLOB` encoding. The Windows production SQLCipher dependency, private raw-key primitive, metadata-only production database-file inspection, guarded read-only SQLCipher connection handoff, consuming validation and authorization chain, Rust-owned application-startup lifecycle, explicit first-time setup lifecycle, minimal production V1 metadata/bootstrap schema, private unwired full-integrity validation boundary, and private process-local migration-confirmation state foundation are implemented through the current repository state. The first business-schema product behavior and physical design are now approved in documentation and remain limited to `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview`. The repository still has no implemented physical parish/business schema, production migration-confirmation path, migration, backup, restore, recovery, replacement, authentication, parish workflow, or general writable database-operation interface. This foundation does not make the full product launch-ready. The accepted installation-evidence, local-volume, device-property, and controlled-host evidence remains otherwise unchanged.

## 2. Authority and objective

The current documentation objective records the final approved first business-schema product and physical-design contract without implementing it. The scope is exactly `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview`, physically designed as `service_requests`, `request_schedule_occurrences`, and `request_cancellation_reviews`. This stage approves the documented table/column shapes, SQL storage classes and codes, row-level constraints, foreign keys, uniqueness, and indexes. It does not assign a schema version, design or implement a migration, implement SQL, create a SQL file, or add Rust business types, IPC, forms, or workflows. The existing Rust-owned migration-confirmation state/capability foundation remains implemented as previously accepted, but no production path can establish its pending opportunity or accept confirmation. Migration remains separately gated, and no claim is made that this future schema is V2.
...
This stage records the final first business-schema product and physical-design decisions in documentation only. The guarded read-only SQLCipher startup chain, operational activation, explicit first-time setup, encrypted database creation, minimal V1 metadata/bootstrap initialization, private unwired full-integrity validation boundary, and private migration-confirmation state foundation remain the implemented baseline. The approved physical design, Rust business transactions, schema-version change, forward migration, recovery, replacement, backup/restore, authentication, parish workflows, release validation, and any broader database-operation interface remain unimplemented. Startup has no setup, migration, full-integrity, repair, recovery, or reset fallthrough.

## 7. Allowed scope

Documentation changes only in `PLANS.md`, `docs/architecture.md`, `docs/product-decisions.md`, `docs/security-and-data.md`, `docs/verification.md`, and `docs/project-overview.md`; `docs/review-checklist.md` changes only if required to avoid a material contradiction. No production code, test, dependency, configuration, SQL, schema, migration, Rust business type, frontend form, IPC, or runtime change is included.

## 8. Prohibited scope

This stage authorizes no dependency, configuration, workflow implementation, maintenance execution, authentication, physical-schema implementation, migration, executable SQL or PRAGMA change, writable database access, freshness redesign, recovery integration, backup/restore, replacement, Tauri command, frontend change, generated inventory, header mutation, or destructive operation. It records the approved physical table/column shapes, SQL storage classes and persisted codes, row-level constraints, foreign keys, uniqueness, and indexes only as documentation. It defines no schema version, schema V2, migration plan or migration SQL, Rust business type, form, or IPC. It also does not define migration eligibility ownership, a migration coordinator, authentication roles, backup implementation or acceptance, writable maintenance owner, exclusivity, migration execution or update ordering, interruption/restart execution policy, or authority for restore, recovery, rekey, replacement, or cleanup.

## 9. Dependency approvals

The accepted Windows production dependency is exactly `rusqlite = { version = "=0.39.0", default-features = false, features = ["bundled-sqlcipher-vendored-openssl"] }` under `[target.'cfg(windows)'.dependencies]`. There is no Windows `rusqlite` development dependency and no direct `libsqlite3-sys` dependency. The minimal V1 metadata/bootstrap schema and first-time setup are implemented. The first business schema has an approved physical design but no implementation or version. Migration, recovery authority, backup/restore, replacement, retention, cleanup, and broader operational database APIs each require separately scoped approval.
...
React may eventually present or invoke the interactive confirmation flow, but it remains non-authoritative. Rust must establish the pending opportunity and construct and retain any future authority. The approved construction boundary is locked below; it accepts no frontend token or trust-bearing value. Full-integrity proof, writable maintenance ownership, exclusivity, installation/evidence/freshness state, version eligibility, and backup proof remain separately composed prerequisites. This policy does not approve migration implementation, schema V2, migration SQL, authentication roles, backup implementation, writable maintenance ownership, or exclusivity design.
...
The Rust state machine is implemented and tested before migration execution exists, but no successful production confirmation path exists: only a `cfg(test)` seam can establish `Pending`. A separately approved Rust migration-specific coordinator or context producer is required before any production opportunity can exist. A dormant command that only returns `notPending` is not required, and neither future command is implemented.
...
STILL UNIMPLEMENTED: the repository contains no business tables, executable business DDL, migration, writable business transaction, Rust business type, IPC, form, or workflow. The existing V1 bootstrap remains unchanged. Implementing this contract would require a future schema-version change and an approved forward migration for existing schema-version 1 installations, but the target version identifier remains unapproved. This design is not named V2, assigns no schema version, creates no migration eligibility, and includes no migration SQL or migration-confirmation change.

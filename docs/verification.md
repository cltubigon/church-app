...
## Future startup-authorization implementation acceptance matrix

Startup authorization after freshness is approved architecture only and is not implemented. No manual application or runtime test applies until an operational caller exists. The already accepted freshness evidence above remains unchanged and is not evidence that startup authorization exists.
...
Future source-boundary checks must prove that the implementation introduces no hidden installation-state loading; filesystem, path, file-identity, or sidecar access; SQL or PRAGMA; database access; freshness, correspondence, metadata/header, readability/integrity, or database-file-inspection rerun; anchor loading or normalization; schema or migration work; setup helper use; storage creation; recovery or repair; Tauri command, IPC, or frontend surface; arbitrary `Connection` callback; production success constructor; visibility widening or crate-root bridge; unsafe/FFI; dependency; or Cargo feature. Any deterministic close injection must be `cfg(test)`-only.

The future implementation must also verify private placement at `production_database_connection_handoff/live_metadata_and_header_validation/database_evidence_correspondence_validation/database_freshness_validation/startup_authorization.rs`, declared as a private child by `database_freshness_validation.rs`. Operational database opening remains a later separate consuming boundary and is not designed or verified here. Account/elevation enforcement and any fresh path/sidecar reinspection before operational use also remain separate future decisions.
...
- Implement and verify the approved startup-authorization boundary after possession of `DatabaseFreshnessValidatedProductionDatabaseConnection`, without combining it with operational opening, setup, migration, recovery, or UI work.

...
No production runtime caller, live metadata read, database readability validation, wrong-key classification, SQLCipher cipher-integrity validation, SQLite `quick_check`, or operational database use was manually tested. The accepted evidence therefore establishes a guarded keyed-but-unvalidated handoff, not a validated database or operational opening authority.

## Remaining production database verification gates

The following gates apply only to separately approved future implementation tasks. They are requirements, not current tests or passing evidence. The repository has an accepted guarded keyed-but-unvalidated connection handoff, but no validated or operational production database-opening workflow. The existing `sqlcipher_windows_temporary_encryption_feasibility` test remains historical Windows test-only experiment evidence.
...
- Database readability-and-integrity tests must start by consuming `ProductionReadOnlyDatabaseConnection`. A valid correct-key synthetic SQLCipher fixture must prove the exact private order: `PRAGMA cipher_integrity_check`, normal zero-row completion, `PRAGMA main.quick_check(1)`, exactly one complete SQLite `TEXT` value `ok`, normal end-of-stream, and return of an opaque validated owner retaining the same connection/guard/inspection-proof lifetime unit. The validated owner must expose consuming close and preserve complete ownership on close failure. No separate readability query, `sqlite_master`, metadata/header read, product query, arbitrary SQL, operational caller, frontend, IPC, or Tauri command may appear.
- A wrong-key fixture must produce only `EncryptedDatabaseAuthenticationOrCipherIntegrityFailed`, without asserting or exposing a diagnostic or native code. A controlled ciphertext/HMAC corruption fixture must produce that same combined category. Cipher-row tests must provide multiple synthetic diagnostic rows, prove the first row alone establishes failure, prove its text is never decoded, copied, formatted, retained, or logged, prove later rows are not required, and prove the row stream is dropped/reset before close.
- Narrow injected quick-check seams must cover a non-`ok` text row, zero rows, multiple rows, non-text output, malformed shape, a stepping error, interruption, and premature termination. Only one complete `ok` text row followed by normal completion may succeed; all other cases must map to `SQLiteReadabilityOrIntegrityFailed`, `ValidationUnavailable`, or `ValidationInterruptedOrIncomplete` as approved and must not expose diagnostic text. A real quick-check-only corruption fixture may be replaced by this injected seam rather than unsafe engine-internal rewriting needed to preserve valid cipher HMACs.
- Unavailable and interrupted seams must prove the exact coarse categories `ValidationUnavailable` and `ValidationInterruptedOrIncomplete`, no success without the complete required terminal result, and no active cancellation or rusqlite hooks API. Redaction tests must exclude SQL or PRAGMA text, cipher or quick-check diagnostics, paths, keys, identifiers or identities, metadata, SQLite/native codes, raw handles, and rusqlite errors or chains.
- Validation-failure close tests must cover each primary category. Close success returns that primary category. Close failure must preserve the same category plus an opaque consuming close-retry owner holding the complete connection, guard, and inspection proof; repeated failure retains them again and retry success releases them without exposing raw close detail. These are future requirements, not current passing evidence.
...
Carlo must manually review the approval record; the metadata-only inspector versus the separate lifetime guard; the inspected proof, guard, SQLite handle, and owner identity/lifetime chain; the exact one-open flags and `win32` VFS; pre-key policy, one key application, and post-key query-only ordering; the bounded ordinary-Windows-open guarantee and exclusions; the keyed-but-unvalidated result; the exact cipher-then-quick-check architecture and absence of a separate readability query; the four combined coarse categories; first-cipher-row disposal and exact quick-check row shape; fail-closed incompletion without active cancellation; same-connection/no-explicit-transaction policy; primary-category preservation on close failure; synchronous whole-file workload acceptance only while no operational caller exists; the still-later metadata/correspondence/freshness and startup-authority boundaries; and confirmation that no implementation entered this documentation task.

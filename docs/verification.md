...
- Future maintenance-authorization verification must prove that production database migration is the sole initial authorization identity, with no generic `MaintenanceOperation` enum or broad generic `MaintenanceAuthorization` capability. Only an explicit interactive confirmation initiated from Church App by the local human operator under the dedicated standard Windows production account may authorize one migration. The boundary represents consent rather than application identity authentication; is single-use, process-local, non-persistent, non-renewing, and operation-scoped; cannot be inferred, automatically granted, granted on startup or setup completion, granted because migration is required, retained across restart, serialized, persisted, reconstructed from frontend-supplied Boolean/string/path/version values, reused for another operation, or used to trigger automatic execution.
- Before any future `ProductionDatabaseMigrationAuthorization` token implementation, verification design requires separately approved evidence that its Rust-owned construction boundary represents the interactive operator confirmation. It must reject ceremonial construction from only a Boolean, enum, version, path, startup state, `Ready` state, backup proof, integrity proof, or existing database owner. React remains non-authoritative.
...
- The product-level taxonomy decision does not yet supply a Rust authorization type or representation, schema V2, migration SQL, IPC shape, UI wording or dialog design, authentication roles, backup implementation, writable maintenance owner, exclusivity design, migration execution, or detailed test fixtures for those future designs.

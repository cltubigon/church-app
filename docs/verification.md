...
Future Layer E tests may use the manifest only for candidate recognition and inventory behind a trusted selection boundary. They must prove fixed-name location, parsing/validation, identifier extraction, size/digest verification, and manifest/envelope identity comparison cannot authorize restore or database replacement. The implemented topology, external/disconnectable eligibility, physical-device separation, and retained exact-root NTFS authority remain non-authorizing prerequisites. The operator will select the exact external volume root through a future unresolved native picker; arbitrary folders do not qualify. The fixed future child is `church-app-recovery-set`. Unrelated files elsewhere on the selected volume are allowed, but any exact or case-conflicting pre-existing child must fail closed and must not be adopted, merged, resumed, repaired, overwritten, or automatically deleted. Manifest file and fixed-child creation, native picker implementation, publication of all three artifacts, create-new/final-name durability, independent destination reopening, first and second complete sets, a two-set success owner, Layer D lifecycle composition and normal product reachability, operational recovery sets, restore, retention/deletion, and migration execution remain unimplemented.

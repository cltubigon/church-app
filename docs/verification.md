...
Focused deterministic lifecycle tests and source-contract checks cover both picker dispatches through `run_on_main_thread`, real `main` window/`HWND` resolution, reuse of `select_native_recovery_volume_root`, one-shot shutdown/main-thread arbitration, lock release before native work, worker-held source/root/exclusivity ownership, retryable `Cancelled` and `Unavailable` parking, and explicit private retry without automatic redisplay. First-root coverage proves canonical production topology, retained-root, and production-separation reuse. Second-root coverage proves canonical retained-root validation followed by the consuming canonical two-root transition, both-root revalidation, first-root preservation only before consumption, custody-only reset on every consuming-transition failure, exact source-plus-two-root success ownership, and unchanged unresolved exit accounting. Capacity-composition coverage proves the exact canonical borrowed required-byte observation, absence of lifecycle byte arithmetic, the narrow canonical capacity facade, pre-consumption source-observation failure preserving both roots, consuming failure retaining neither root, exact source-plus-capacity-owner success, worker-only coarse state, shutdown-before-work suppression, and capacity/root drop before canonical source shutdown. Canonical capacity regressions cover checked arithmetic, equality, one-byte insufficiency, independent first/second rejection and ordering, unavailable observation, source-size mapping, pre/post destination change, redacted errors, and prohibited authority surfaces. The deterministic picker seam displays no UI and uses no Tauri runtime, USB, or browser automation.

Source exclusions cover lifecycle/IPC path exposure, fake production selection, fixed-child creation, publication, custody re-entry, complete-set verification, migration execution, and recovery-device cleanup. No manual capacity run is claimed for this unreachable isolated slice. Future integrated real-device validation must cover free-space observation on both selected devices, insufficient space on either device, storage change between selection and capacity checking, shutdown before and around the capacity stage, and confirmation that failed validation creates no recovery-set directory. Picker validation also remains required for device-2 real parenting/modality; Cancel/X/Alt+F4; retry after cancellation/unavailability; valid distinct and same-device cases; detach races; Windows 10/11; and keyboard-only behavior.

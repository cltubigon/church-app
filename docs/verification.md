...
## First recovery-volume lifecycle evidence

Focused deterministic lifecycle tests and source-contract checks cover first-picker `run_on_main_thread` dispatch, real `main` window/`HWND` resolution, reuse of `select_native_recovery_volume_root`, one-shot shutdown/main-thread arbitration, lock release before native work, worker-held source/exclusivity, retryable `Cancelled` and `Unavailable` parking, explicit private retry without automatic redisplay, canonical production topology projection through the retained source, canonical retained-root and production-separation reuse, successful source-plus-first-root retention, unchanged unresolved exit accounting, and root-drop-before-canonical source shutdown. The deterministic picker seam displays no UI and uses no Tauri runtime, USB, or browser automation.

Source exclusions cover lifecycle/IPC path exposure, fake production selection, a second picker, capacity validation, fixed-child creation, publication, custody re-entry, migration execution, and recovery-device cleanup. Manual validation remains required for real Church App parenting/modality, Cancel/X/Alt+F4, explicit retry after Cancel and reproducible native unavailability, shutdown while visible, internal production volume, external NTFS USB, child rejection, detach before retained-root validation, same-device rejection, and Windows 10/11.

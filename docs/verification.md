...
Layer D verification must eventually begin only from the existing keyless custody-verified backup owner and must prove exactly two complete digital recovery sets on two physically distinct, physically disconnectable external devices, both distinct from production storage. It must reject two logical locations on one physical device and fail closed on ambiguous, unavailable, inconsistent, substituted, or unsupported topology. Each destination is independently selected and validated, remains associated with the same device through publication, and is independently reopened for final verification. Test-only local-volume and device-property candidate classifiers cannot satisfy this production proof.

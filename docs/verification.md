...
Future Layer E tests may use the manifest only for candidate recognition and inventory behind a trusted selection boundary. They must prove fixed-name location, parsing/validation, identifier extraction, size/digest verification, and manifest/envelope identity comparison cannot authorize restore or database replacement. The implemented topology, external/disconnectable eligibility, physical-device separation, retained exact-root NTFS authority, and opaque production-separated retained-root composition remain non-authorizing prerequisites. Focused source/deterministic tests require production revalidation before complete-root revalidation and reuse of the existing identity comparison, preserve exact root ownership, revalidate both sides, reject same-device observations, redact owners/errors, and expose no path, handle, topology, eligible-device, or disk-number getter. The operator will select the exact external volume root through a future unresolved native picker; arbitrary folders do not qualify. The fixed future child is `church-app-recovery-set`. Unrelated files elsewhere on the selected volume are allowed, but any exact or case-conflicting pre-existing child must fail closed and must not be adopted, merged, resumed, repaired, overwritten, or automatically deleted. Set-2 retained-root composition, manifest file and fixed-child creation, native picker implementation, publication of all three artifacts, create-new/final-name durability, independent destination reopening, first and second complete sets, a two-set success owner, Layer D lifecycle composition and normal product reachability, operational recovery sets, restore, retention/deletion, and migration execution remain unimplemented.

...
The boundary has no production caller and is not wired into startup, setup, application lifecycle, migration, backup, restore, recovery, IPC, or frontend. Ordinary startup still performs only `PRAGMA cipher_integrity_check` followed by `PRAGMA main.quick_check(1)`; setup remains unchanged. This evidence establishes one reusable prerequisite capability only. It does not establish migration eligibility or authorization, a verified recoverable backup, writable maintenance ownership, migration exclusivity, a schema-version-2 implementation, interruption/restart policy, update ordering, or any migration call site, and it does not decide whether future maintenance must repeat cipher-integrity validation before mutation.
...
- Future migration verification remains gated on separately approved composition of the implemented full-integrity capability with independently established eligibility for the exact 1-to-2 compatibility pair and a migration plan, one migration-specific operator authorization, verified recoverable encrypted backup proof, completed full-integrity validation at the migration boundary, a separate writable maintenance connection/owner, downgrade refusal, interruption/restart classification, exact metadata/`user_version`/evidence/freshness/lineage update ordering, concurrency/exclusivity, close-failure ownership, and proof that migration cannot become startup/setup fallback. Observing schema 1 and possessing `ProductionDatabaseMigrationAuthorization` must never by themselves imply eligibility or execution. Migration remains unapproved and unimplemented; this document does not define the detailed test design for those prerequisites.

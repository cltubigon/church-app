...
## Accepted production SQLCipher dependency and raw-key primitive evidence

The exact Windows production dependency promotion and private raw-key primitive are implemented and accepted. Clean CI passed at commit `58e903bf7f1ac186a5172a8be96a5d5cf66d8b58`. The accepted local validation categories are the exact dependency/private-source contract, deterministic raw-key encoding and boundary coverage, owned-buffer zeroization behavior, single-call success and coarse failure behavior, null-handle refusal, Rust formatting, Clippy with warnings denied, full locked Rust tests, and repository diff checks. The historical feasibility test remains separate evidence of the earlier candidate evaluation.

No application runtime or operational production database behavior was manually tested for this gate. In particular, the accepted primitive tests do not open, create, select, query, inspect, mutate, rekey, or close a database; validate a real handle; establish key correctness or database readability; read metadata; run SQLCipher or SQLite integrity checks; or exercise setup, startup, recovery, frontend, Tauri commands, or IPC.

## Remaining production database verification gates

The following gates apply only to separately approved future implementation tasks. They are requirements, not current tests or passing evidence. The repository has a production SQLCipher dependency and private raw-key application primitive, but no complete operational production database-opening workflow. The existing `sqlcipher_windows_temporary_encryption_feasibility` test remains historical Windows test-only experiment evidence.
...
- Database-open trust-chain tests must prove the separation and ordering of production file inspection, trusted generation-bound key recovery, controlled connection opening, exactly one application through the accepted primitive, and later database validation. Opening tests must prove read-only, no-create behavior on an absent path and existing encrypted database; a single bounded attempt; no writable fallback, migration, repair, journal recovery, key regeneration, sidecar creation, or `immutable=1`; maximum five-second busy handling; defensive/query-only policy; disabled extension loading; `trusted_schema` off; one consistent snapshot; and explicit close outcomes.
- Wrong-key and corruption tests must distinguish coarse wrong-key/unreadable-encrypted, corrupt, unsupported, integrity-failed, and unavailable outcomes without revealing paths, SQL/PRAGMA text, keys, identifiers, metadata, native codes, or raw chains. Every open must prove cipher-integrity and limited `quick_check`; full checks must fail on cancellation or incompletion.
...
Carlo must manually review the approval record, all metadata fields and encodings, the UTF-8 application identifier versus 16-byte BLOB database-format identity distinction, historical feasibility evidence versus the current Windows production dependency and private primitive, the absence of an operational database-opening caller or flow, database/recovery key separation, freshness limitation, read-only/no-create behavior, path/sidecar and journal policy, migration/recovery authority separation, the next database-open trust-chain task, and confirmation that no implementation entered this documentation task.

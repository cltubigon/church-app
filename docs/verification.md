...
The implemented evidence begins with `VerifiedEncryptedProductionDatabaseMigrationBackupStage`: exact full-integrity source and same database key, independent cipher and full SQLite integrity, exact metadata/header equality, ciphertext at rest, unchanged retained source, retained source-bound migration authorization, and retained parent/leaf identity continuity. The private consuming integration establishes `VerifiedRecoveryEnvelopedProductionDatabaseMigrationBackup`, including independently verified envelope authentication, recovered-key use against the exact retained stage, and success ownership of generated recovery-key material for the implemented custody foundation. That owner does not by itself establish an executed human custody ceremony, operational survival after process loss, complete durable Layer D publication, two verified complete recovery sets, restore, retention, or migration execution. Later implemented work publishes and freshly verifies the first set's database and envelope payloads and its manifest-last manifest, performs recovered-key verification, verifies the exact three-entry layout, independently verifies the first complete recovery set, and publishes plus freshly verifies the second set's database, recovery envelope, and manifest-last artifact from the original retained sources. Independent exact second-directory layout verification, second complete-set verification, and final two-set Layer D capability remain unimplemented.
...
The implemented comparisons reject two logical locations on one physical device and fail closed when any participating topology or eligibility revalidation is unavailable, changed, inconsistent, malformed, unsupported, or no longer eligible. They expose no raw disk number, handle, path, generic comparison callback, serialization, frontend command, or Tauri surface. The retained root owner requires the same exact root and full identity, valid disk/directory/non-reparse/non-delete-pending facts, NTFS, and revalidated external eligibility. First-generation recovery volumes are NTFS only; exFAT is deferred and FAT32 is categorically rejected. First-set database, envelope, and manifest-last publication, recovered-key verification, exact three-entry layout verification, independent first complete-recovery-set verification, and second-set database, recovery-envelope, and manifest-last publication are implemented. Each second-artifact transition independently revalidates the retained source, both destinations, and represented production/recovery plus recovery/recovery separation before and after publication. Native picker selection, independent exact second-directory layout verification, second complete-set verification, final two-set Layer D capability, Layer D lifecycle integration, restore, and migration execution remain unimplemented. Test-only local-volume and device-property candidate classifiers remain test-only and cannot satisfy full Layer D success.

For each set, future tests must prove the fixed final layout contains exactly `parish-data.db`, `migration-recovery-envelope-v1.bin`, and `recovery-set-v1.manifest`; the names are fixed constants, not input or encoded fields, and file presence alone is insufficient. The accepted committed Recovery Set Manifest V1 pure-codec tests prove exact length 98; magic `CHLDRSM\0`; big-endian `u16` version `00 01`; raw identifier, big-endian database length, raw database digest, and raw envelope digest at `0..8`, `8..10`, `10..26`, `26..34`, `34..66`, and `66..98`; and rejection of truncation, trailing data, wrong magic, and unsupported version under exactly `WrongTotalLength`, `WrongMagic`, and `UnsupportedVersion`.
...
Accepted codec redaction tests prove outward formatting omits complete identifiers/digests, manifest bytes, database length, observed attacker-controlled values, paths, and device/destination identity. Construction tests prove only in-memory derivation from the retained source; they create no file, destination, or publication capability and preserve the owner for later work or shutdown. The private recovered-key prerequisite accepts only the first-set publication owner plus one bounded owned custody-record entry and establishes the keyless predecessor. The implemented complete-set transition accepts only that predecessor, uses a fixed three-slot exact-name classifier and two native directory enumerations, freshly reopens and identity-binds all three artifacts, requires exact 98-byte and 182-byte reads with clean EOF, streams the database through exact length/SHA-256/EOF verification, checks all manifest/database/envelope/backup-set/source correspondence, and revalidates source plus destination authority before success. Focused pure/source tests cover complete/missing/fourth/case-conflict/duplicate/dot/malformed/termination/close cases and narrow redacted ownership. The genuine synthetic Windows fixture now traverses the full first-set publication, recovered-key, exact-layout, and complete-set path, then independently publishes the second database, envelope, and manifest-last artifact. It proves the second manifest is the exact 98-byte canonical reconstruction from the retained original source, set 1 is not used as its source, earlier artifacts remain unchanged, and create-new conflicts rather than replacing. Independent exact second-directory layout verification, the second complete-set proof, and final Layer D success remain future work; one complete recovery set plus three individually verified second-set artifacts cannot establish Layer D completion.

Future Layer E tests may use the manifest only for candidate recognition and inventory behind a trusted selection boundary. They must prove fixed-name location, parsing/validation, identifier extraction, size/digest verification, and manifest/envelope identity comparison cannot authorize restore or database replacement. The implemented topology, external/disconnectable eligibility, physical-device separation, retained exact-root NTFS authority, opaque two-root composition, capacity prerequisite, fixed-child creation, publication, recovered-key proof, first complete-set proof, and second artifact proofs remain non-authorizing beyond their named facts. The operator will select each exact external volume root through a future unresolved native picker; arbitrary folders do not qualify. The first complete set and second database, recovery-envelope, and manifest-last artifacts are implemented; native picker implementation, independent exact second-directory layout verification, second complete-set verification, a two-set Layer D success owner, lifecycle composition and normal product reachability, operational recovery, restore, retention/deletion, and migration execution remain unimplemented.
...
- Future migration verification now starts from implemented opportunity/revalidation, source-bound authorization, full-integrity source, dedicated exclusivity, verified encrypted SQLCipher backup-stage, recovery-envelope verification, C1, C2a, private C2b lifecycle dispatch, the private Recovery Set Manifest V1 codec, trusted construction, custody-owner fact observation, the complete first set, and second-database, second-envelope, and second-manifest-last publication with independent fresh verification against the retained source. Layer D architecture remains approved but the native picker, independent exact second-directory layout verification, second complete-set verification, the strongest supported namespace/directory durability proof, two-set lifecycle composition, final Layer D success, and runtime/manual validation remain prerequisites. Migration also remains gated on normal product reachability and manual ceremony validation, a separate writable maintenance connection/owner, interruption/restart classification, exact metadata/`user_version`/evidence/freshness/lineage update ordering, later execution-chain close-failure composition, and proof that migration cannot become startup/setup fallback. Observing schema 1, possessing `ProductionDatabaseMigrationAuthorization`, constructing, parsing, validating, publishing, or freshly verifying a manifest, or later holding Layer D success must never by itself imply eligibility or execution.

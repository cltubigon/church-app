...
Future Layer E tests may use the manifest only for candidate recognition and inventory behind a trusted selection boundary. They must prove fixed-name location, parsing/validation, identifier extraction, size/digest verification, and manifest/envelope identity comparison cannot authorize restore or database replacement. The implemented topology, external/disconnectable eligibility, physical-device separation, retained exact-root NTFS authority, opaque production-separated first-root composition, and opaque two-retained-root composition remain non-authorizing prerequisites. Focused source/deterministic tests require complete set-1 revalidation before complete set-2 revalidation and reuse of the existing generic second-device transition; cover set 2 matching production, set 2 matching set 1, and set 2 distinct from both; fail closed for first-root, second-root, and production revalidation failure; repeat both distinctions; preserve both exact root handles/facts; redact owners/errors; and expose no path, handle, topology, eligible-device, filesystem, comparison boolean, or disk-number getter. The operator will select each exact external volume root through a future unresolved native picker; arbitrary folders do not qualify. The fixed future child is `church-app-recovery-set`. Unrelated files elsewhere on the selected volume are allowed, but any exact or case-conflicting pre-existing child must fail closed and must not be adopted, merged, resumed, repaired, overwritten, or automatically deleted. Capacity/free-space queries, manifest file and fixed-child inspection/creation, native picker implementation, publication of all three artifacts, create-new/final-name durability, independent destination reopening, first and second complete sets, a two-set Layer D success owner, lifecycle composition and normal product reachability, operational recovery sets, restore, retention/deletion, and migration execution remain unimplemented.

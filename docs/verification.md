...
The boundary has no production caller and is not wired into startup, setup, application lifecycle, migration, backup, restore, recovery, IPC, or frontend. Ordinary startup still performs only `PRAGMA cipher_integrity_check` followed by `PRAGMA main.quick_check(1)`; setup remains unchanged. This evidence establishes one reusable prerequisite capability only. It does not establish migration or maintenance authorization, a verified recoverable backup, writable maintenance ownership, migration exclusivity, a target schema/version, schema V2, interruption/restart policy, update ordering, or any migration call site, and it does not decide whether future maintenance must repeat cipher-integrity validation before mutation.
...
## Future first business-schema verification contract

No business schema or runtime behavior is implemented by the current documentation decision. When physical schema design is separately proposed, verification must trace it back to exactly `ServiceRequest`, `RequestScheduleOccurrence`, and `RequestCancellationReview` without assuming table names, columns, types, codes, indexes, constraints, foreign keys, a schema version, or a migration in advance. It must also prove that the existing V1 metadata/bootstrap and startup, setup, evidence, correspondence, freshness, SQLCipher, migration-authorization, Rust-authority, and React non-authority boundaries remain unchanged.

Future design verification must cover the five fixed categories with no configurable catalog; creation at `Pending`; only the four approved forward status transitions; cancellation review separate from primary status; unchanged `Pending`/`Scheduled` status while review awaits; one unresolved review at a time; approval to `Cancelled`; rejection without primary-status change; retained resolved reviews; later requests after rejection; and dedicated discovery/filtering of unresolved cancellation reviews.

Scheduling verification must cover separate occurrences, separately representable Funeral and Burial facts, exact local date/time conflicts across one shared parish schedule, the `Asia/Manila` invariant, and per-occurrence bounded free-text locations. It must not infer duration, end time, interval overlap, clergy/resource/capacity scheduling, resource domains, a location catalog, location-sensitive conflicts, or required/optional Funeral and Burial cardinality. Wedding/Marriage and First Communion must have final occurrence locations.

Data-minimization verification must limit the request snapshot to full name, mobile/phone, and optional email; require only request `created_at` and review `requested_at`/resolved `resolved_at`; and exclude normalized people/parishioners, identity matching, generic audit/history, soft deletion, and permanent or sacramental records. Internal database identity must remain sufficient without inventing a human-facing request number. All unresolved physical representations and validation/retention details require later approval.

The documentation-only validation for recording this direction is limited to complete diff review, allowed-file review, `git diff --check`, and its cached equivalent. No npm, Cargo, Tauri, application/runtime, or database command is required because there is no executable change.

## Remaining production database verification gates

- The minimal version-1 metadata/bootstrap schema and its setup creation path are implemented. The first business-schema domain scope is now approved only at the conceptual product level; physical schema verification must distinguish it from the bootstrap contract. No schema V2, physical business table, or parish workflow implementation is approved.
...
- Future migration verification remains gated on separately approved composition of the implemented full-integrity capability with exact source/target version eligibility and migration plan, one migration-specific operator authorization, verified recoverable encrypted backup proof, completed full-integrity validation at the migration boundary, a separate writable maintenance connection/owner, forward-only behavior, downgrade refusal, interruption/restart classification, exact metadata/`user_version`/evidence/freshness/lineage update ordering, concurrency/exclusivity, close-failure ownership, and proof that migration cannot become startup/setup fallback. `ProductionDatabaseMigrationAuthorization` possession must never by itself imply eligibility or execution. Migration remains unapproved and unimplemented; this document does not define the detailed test design for those prerequisites.
- Implemented migration-confirmation verification proves that only Rust test code can establish the exact process-local `Pending` opportunity; React cannot create or identify it; no production producer or bare IPC call exists; and the migration-specific substate is owned beneath `ApplicationLifecycle`, separate from `StartupStatus` and any generic maintenance state machine.
...
- The state machine is implemented and tested before execution, but no successful production confirmation path exists until a separately approved Rust migration-specific coordinator or context producer can legitimately establish `Pending`; the current producer is `cfg(test)`-only. A dormant command that only returns `notPending` is not required. Migration eligibility remains separate and must be revalidated when execution is composed.
...
- The product-level decisions now have only the private Rust authorization/state representation. They still do not supply schema V2, migration SQL, production IPC, UI wording or dialog design, authentication roles, backup implementation, writable maintenance owner, exclusivity design, migration execution, or detailed test fixtures for those future designs.

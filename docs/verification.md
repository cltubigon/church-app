...
For each set, future tests must prove the fixed final layout contains the exact retained verified encrypted SQLCipher backup, exact existing recovery envelope, and one bounded non-secret manifest; file presence alone is insufficient. Verification must cover manifest/inventory correspondence, exact migration-backup-set identifier, byte lengths and SHA-256 digests, exact backup and envelope equality with the retained source, envelope-to-database binding, recovered-key verification against the reopened final database, canonical metadata/header checks, canonical integrity appropriate to the existing chain, and destination/device continuity. Exact manifest serialization remains a separately approved design and implementation prerequisite.

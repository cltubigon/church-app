...
- Future migration-confirmation verification must prove that Rust establishes an exact process-local `Pending` opportunity before confirmation; React cannot create or identify it; a bare call creates no authority; and the owner is `ApplicationLifecycle` or a migration-specific substate beneath it rather than `StartupStatus` or a generic maintenance state machine.
- State-transition tests must cover `NotOffered -> Pending -> Authorized -> Consumed` and `Pending -> Revoked`, with atomic confirmation and cancellation. After confirmation, cancellation, revocation, invalidation, or consumption, no second `Pending` may be created in the same process. A fresh process is required, and restart must destroy pending state and unconsumed authorization. Shutdown must revoke `Pending` before or atomically with stopping, and Rust-detected migration-context or eligibility invalidation must revoke it.
...
- No `ConfirmedMigrationIntent` abstraction is approved. Future confirmation should directly construct and internally retain private, non-`Copy`, non-`Clone`, non-serializable, non-persistent, redacted, exactly-once-consumable `ProductionDatabaseMigrationAuthorization`. That type remains unimplemented.
...
- The state machine may later be implemented and tested before execution, but no successful production confirmation path may exist until a separately approved Rust migration-specific coordinator or context producer can legitimately establish `Pending`. A dormant command that only returns `notPending` is not required. Migration eligibility remains separate and must be revalidated when execution is composed.
...
- The product-level decisions do not yet supply a Rust authorization type or representation, schema V2, migration SQL, complete IPC representation beyond the locked command/result direction, UI wording or dialog design, authentication roles, backup implementation, writable maintenance owner, exclusivity design, migration execution, or detailed test fixtures for those future designs.

...
The implemented comparisons reject two logical locations on one physical device and fail closed when any participating topology or eligibility revalidation is unavailable, changed, inconsistent, malformed, unsupported, or no longer eligible. Lifecycle integration now reaches exact worker-held `FirstCompleteRecoverySetVerified` through explicit native re-entry, canonical recovered-key verification, verifier-close-only retry, and canonical complete-set verification/retry. Focused static and ownership tests cover the main-thread HWND path, retryable cancellation/unavailability, wrong-record classification, shutdown ordering, secret-free `LifecycleInner`, filesystem non-mutation, and the absence of set-2 or migration execution calls. Manual Windows dialog behavior remains unvalidated. Set-2 and final aggregate lifecycle integration, normal product reachability, restore, and migration execution remain unimplemented.
...
Accepted focused tests now traverse both complete-set proofs and the final aggregate transition. Lifecycle source-contract tests additionally establish private main-thread native recovery-key re-entry, real-main-window HWND resolution, typed submitted ownership, retryable cancellation/unavailability, canonical recovered-key verification, wrong-record fresh retry, close-only verifier resolution, canonical complete-set verification/retry, keyless success parking, source-only shutdown abandonment, unresolved migration work, retained exclusivity, and no set-2 or migration-execution call. These source-contract tests do not establish runtime dialog behavior; manual Windows validation remains future work.

...
- Future migration verification remains gated on separately approved composition of the implemented full-integrity capability with an exact target schema/version, explicit maintenance authorization, verified recoverable encrypted backup, completed full-integrity validation at the migration boundary, a separate writable maintenance connection/owner, forward-only behavior, downgrade refusal, interruption/restart classification, exact metadata/`user_version`/evidence/freshness/lineage update ordering, concurrency/exclusivity, close-failure ownership, and proof that migration cannot become startup/setup fallback. Migration remains unapproved and unimplemented; this document does not define the detailed test design for those prerequisites.
- Future maintenance-authorization verification must prove that only an explicit interactive confirmation initiated from Church App by the local human operator under the dedicated standard Windows production account can authorize one maintenance operation. It must prove the boundary represents consent rather than application identity authentication; is single-use, process-local, non-persistent, non-renewing, and operation-scoped; cannot be inferred, automatically granted, granted on startup or setup completion, granted because migration is required, retained across restart, serialized, persisted, reconstructed from frontend-supplied Boolean/string/path/version values, or reused for another operation; and cannot trigger automatic maintenance execution.
- Future authority-composition verification must prove that application start, `Ready`, setup authority, startup authorization, `OperationalProductionDatabase`, `FullIntegrityValidatedProductionDatabaseConnection`, backup existence or verified-backup proof, writable ownership, migration eligibility, version compatibility, and concurrency or exclusivity cannot substitute for maintenance authorization. It must also prove React is non-authoritative and that Rust constructs and owns the future authority independently of the separate backup, integrity, version, writable-ownership, exclusivity, installation, evidence, and freshness gates.
- The product-level trigger decision does not yet supply a Rust `MaintenanceAuthorization` type, maintenance-operation enum or taxonomy, source/target version pair, schema V2, IPC shape, UI wording or dialog design, authentication roles, backup implementation, writable maintenance owner, migration execution, or detailed test fixtures for any of those future designs.

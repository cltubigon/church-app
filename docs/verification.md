...
Focused deterministic lifecycle tests and source-contract checks cover both picker dispatches through `run_on_main_thread`, real `main` window/`HWND` resolution, reuse of `select_native_recovery_volume_root`, one-shot shutdown/main-thread arbitration, lock release before native work, worker-held source/root/exclusivity ownership, retryable `Cancelled` and `Unavailable` parking, and explicit private retry without automatic redisplay. First-root coverage proves canonical production topology, retained-root, and production-separation reuse. Second-root coverage proves canonical retained-root validation followed by the consuming canonical two-root transition, both-root revalidation, first-root preservation only before consumption, custody-only reset on every consuming-transition failure, exact source-plus-two-root success ownership, unchanged unresolved exit accounting, and roots dropped before canonical source shutdown. The deterministic picker seam displays no UI and uses no Tauri runtime, USB, or browser automation.

Source exclusions cover lifecycle/IPC path exposure, fake production selection, capacity validation, fixed-child creation, publication, custody re-entry, complete-set verification, migration execution, and recovery-device cleanup. Manual validation remains required for device-2 real picker parenting/modality; Cancel/X/Alt+F4; retry after cancellation/unavailability; a valid distinct second USB device; the same physical device selected for both slots; two volumes on one physical device; device 1 detached while selecting device 2; device 2 detached before retained-root validation; shutdown while the second picker is visible; Windows 10/11; and keyboard-only behavior.

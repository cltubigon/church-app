...
- Database readability and integrity-validation tests must start from the opaque keyed-but-unvalidated owner and distinguish coarse wrong-key/unreadable-encrypted, corrupt, unsupported, integrity-failed, and unavailable outcomes without revealing paths, SQL/PRAGMA text, keys, identifiers, metadata, native codes, handles, or raw chains. They must prove SQLCipher cipher-integrity checking and limited SQLite `quick_check`; full checks must fail on cancellation or incompletion.
...
Carlo must manually review the approval record; the metadata-only inspector versus the separate lifetime guard; the inspected proof, guard, SQLite handle, and owner identity/lifetime chain; the exact one-open flags and `win32` VFS; pre-key policy, one key application, and post-key query-only ordering; the bounded ordinary-Windows-open guarantee and exclusions; the keyed-but-unvalidated result; the absence of a runtime caller and validated or operational opening authority; the later readability/integrity boundary; the still-later metadata/correspondence/freshness and startup-authority boundaries; and confirmation that no implementation entered this documentation task.

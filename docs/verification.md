...
This recovery-envelope documentation stage changes none of those runtime behaviors. Its validation is limited to complete documentation diff review, allowed-file review, baseline/final Git-state review, and `git diff --check`. No npm, Cargo, Tauri, application/runtime, SQL/database, migration, key-generation, or envelope command is appropriate because there is no executable change.

## Locked migration recovery-envelope verification contract (not implemented)

Existing implementation evidence establishes only `VerifiedEncryptedProductionDatabaseMigrationBackupStage`: exact full-integrity source and same database key, independent cipher and full SQLite integrity, exact metadata/header equality, ciphertext at rest, unchanged retained source, and retained source-bound migration authorization. It does not establish portable recoverability, recovery-key custody, durable publication, restore, or retention authority.

Future format tests for Migration Recovery Envelope Format Version 1 must prove exact XChaCha20-Poly1305 identity, a 192-bit nonce, and the construction's fixed untruncated 128-bit tag. Version 1 rejects plain ChaCha20, 96-bit-nonce ChaCha20-Poly1305, AES-CBC/CTR, unauthenticated or HMAC-only protection, DPAPI framing as the portable envelope, SQLCipher payload misuse, unknown versions, unknown algorithms, negotiation, and fallback.

Key-generation tests must prove a separately generated uniformly random 256-bit migration recovery key from the approved OS source; a dedicated non-`Clone`, non-`Copy`, non-serializable, redacted, best-effort-zeroizing owner; process-local plaintext lifetime; one independent nonzero 128-bit recovery-key-generation identifier; and one independent nonzero opaque 128-bit backup-set identifier. The identifiers are public and not key material, and the backup-set identifier never reuses setup-publication, installation, database-key-generation, or recovery/replacement generation. Randomness failure fails closed.

Nonce tests must prove one independently OS-random 192-bit nonce per envelope, encoded as non-secret framing, with no counter, global persisted allocator, or deterministic derivation from identifiers, digest, timestamp, path, metadata, or key material. Reuse under the same recovery key is prohibited.

Payload tests must prove the plaintext contains exactly the 256-bit production database key, 128-bit database-key-generation identifier, 128-bit backup-set identifier, and 256-bit SHA-256 digest of the exact encrypted backup-stage database bytes. Hashing occurs only after successful writer close and current verified-stage proof. Tests must reject the inclusion of parish, installation, installation-generation, recovery/replacement-generation, setup-publication, `application_id`, `user_version`, schema-version, database-format-identity, evidence/freshness keys, active DPAPI wrapper, path, or timestamp data absent a later approved format version.

AAD/domain tests must prove one fixed explicit Church App migration-recovery-envelope separator, format version, algorithm identifier, recovery-key-generation identifier, and any backup-set identifier framed outside ciphertext are authenticated. Evidence-envelope and freshness-envelope domains cannot verify as this domain.

Parsing and opening tests must prove authentication-before-release: framing, exact version, exact algorithm, structurally valid required identifiers, ciphertext length, and AEAD authentication all precede release of any production database-key plaintext. A wrong recovery key or authentication failure releases no candidate, and no unauthenticated plaintext inspection is possible. After opening, tests require payload/envelope backup-set equality, database-key-generation correspondence to recovered database metadata, and fresh exact-stage SHA-256 equality.
...
Independent recovery verification must exercise this exact order while the dedicated migration-exclusivity owner remains held: consume `VerifiedEncryptedProductionDatabaseMigrationBackupStage`; generate key, generation identifier, set identifier, and nonce; hash the exact closed stage; independently recover the production key from the active wrapper for construction; seal; destroy/drop that wrapper-derived owner; independently parse/authenticate/decrypt the envelope without reloading the live wrapper; reconstruct a fresh generation-bound candidate; re-hash and match the stage; reopen staged SQLCipher using only that candidate; rerun canonical cipher integrity; rerun canonical fixed metadata/header validation; explicitly close; then return envelope-stage success.

Full SQLite integrity must not be repeated during that verifier when the prior encrypted stage passed full integrity, authenticated SHA-256 equality proves byte identity, recovered-key cipher integrity succeeds, and exact metadata/header equality succeeds. If byte equality cannot be established, verification fails; it does not substitute a weaker check. Tests must also prove the transition never mutates the staged database.

DPAPI tests, if a local convenience key copy is later approved, must prove that copy is not the portable envelope and is never accepted as sufficient portable custody. Envelope construction preserves recovery/replacement generation. Authority tests must prove the recovery key and envelope grant no migration/startup authorization, database write, restore, evidence/freshness, or mutation authority; success retains exact migration authorization/source/stage/envelope ownership; and primary failure destroys migration authorization under canonical source-close rules.

The immediate verified type should be named `VerifiedRecoveryEnvelopedProductionDatabaseMigrationBackup` or a conventionally equivalent term, never `VerifiedRecoverableProductionDatabaseBackup`. Verification must keep encrypted-stage, envelope, custody, publication, restore, and retention/deletion layers separate. Exact crypto dependency, custody, ten-code applicability/format, two-copy meaning, final root/filenames, publication atomicity/durability, rotation, supersession, retention, destructive deletion, and restore tests remain deferred. This contract approves no Cargo change or dependency.
...
- Future migration verification now starts from implemented opportunity/revalidation, source-bound authorization, full-integrity source, dedicated exclusivity, and verified encrypted SQLCipher backup-stage foundations. It remains gated on the unimplemented recovery-envelope verification contract above, recovery-key custody and durable publication, a separate writable maintenance connection/owner, downgrade refusal, interruption/restart classification, exact metadata/`user_version`/evidence/freshness/lineage update ordering, close-failure composition, and proof that migration cannot become startup/setup fallback. Observing schema 1 and possessing `ProductionDatabaseMigrationAuthorization` must never by themselves imply eligibility or execution.
...
- The migration foundation includes the private Rust authorization/state representation, production opportunity/revalidation, exclusivity, full-integrity-bound source, and encrypted backup stage. The separately documented first-business-schema physical contract supplies target schema version 2 and the 1-to-2 compatibility direction, but there is no migration SQL, production confirmation IPC/UI, authentication role, recovery envelope, recovery-key custody, durable publication, writable maintenance owner, migration execution, or detailed fixtures for those future designs.
- Backup and staged-restore tests must preserve the verified SQLCipher-only encrypted stage, use the separately authenticated XChaCha20-Poly1305 recovery envelope specified above, establish custody and publication in later contracts, retain exact correspondence and lineage policy, require explicit recovery authority, and permit no production plaintext database or fallback.

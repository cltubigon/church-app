...
Accepted codec redaction tests prove outward formatting omits complete identifiers/digests, manifest bytes, database length, observed attacker-controlled values, paths, and device/destination identity. Construction tests prove only in-memory derivation from the retained source; they create no file, destination, or publication capability and preserve the owner for later work or shutdown. Future integration tests must independently reopen and prove actual envelope length 182, actual database length and both SHA-256 digests against the manifest, manifest/envelope backup-set identity, exact backup/envelope correspondence with the retained source, authenticated envelope binding, recovered-key verification, canonical database verification, and device continuity/separation. Set 2 must be independently published from the retained source. Only the full future transition may produce Layer D success; manifest presence, construction, parsing, validation, or digest equality cannot do so.

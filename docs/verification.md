...
Accepted focused tests now traverse both complete-set proofs. The second transition accepts only `FirstCompleteRecoverySetAndSecondRecoverySetArtifactsPublished` plus a fresh owned custody record, reuses the fixed three-slot exact-name classifier for two native directory observations, freshly reopens and retained-identity-binds the exact 98-byte manifest, exact 182-byte envelope, and second database, authenticates the actual fresh envelope, and sends only the fresh second database to the canonical recovered-key SQLCipher verifier. The genuine synthetic Windows fixture proves cipher integrity, fixed metadata/header correspondence, explicit close, unchanged first- and second-set artifacts, and exactly three canonical second-directory entries. Focused failures cover a checksum-valid wrong key, changed freshly read second-envelope bytes, changed layout observation, and verifier-close retry ownership. Success and ordinary failure remain keyless; retry after ordinary failure requires a fresh record. Final two-set Layer D success remains future work; two independently complete sets do not yet establish the final Layer D capability.

...
Accepted codec redaction tests prove outward formatting omits complete identifiers/digests, manifest bytes, database length, observed attacker-controlled values, paths, and device/destination identity. Construction tests prove only in-memory derivation from the retained source; they create no file, destination, or publication capability and preserve the owner for later work or shutdown. The private recovered-key prerequisite now accepts only the first-set publication owner plus one bounded owned custody-record entry; reuses strict checksum parsing; matches both retained-envelope identifiers before material construction; freshly reopens and identity-binds the exact 182-byte published envelope; authenticates it; proves the freshly reopened database's exact retained identity, length, SHA-256, path, volume, and source correspondence; and runs the canonical read-only SQLCipher cipher-integrity and metadata/header checks with explicit close. Focused tests cover bounded/redacted secret ownership, malformed input, checksum mismatch, both association mismatches, keyless outward owners, the two-input signature, and composition of the existing cryptographic, fresh-reopen, hashing, and SQLCipher primitives. The broader runtime fixture was deliberately not expanded in this slice. Exact three-entry layout verification, the first complete-set owner, set 2, and final Layer D success remain future work; recovered-key verification alone cannot establish a complete recovery set.

Future Layer E tests may use the manifest only for candidate recognition and inventory behind a trusted selection boundary. They must prove fixed-name location, parsing/validation, identifier extraction, size/digest verification, and manifest/envelope identity comparison cannot authorize restore or database replacement. The implemented topology, external/disconnectable eligibility, physical-device separation, retained exact-root NTFS authority, opaque two-root composition, capacity prerequisite, fixed-child creation, and first-set database/envelope/manifest publication remain non-authorizing. The operator will select each exact external volume root through a future unresolved native picker; arbitrary folders do not qualify. Capacity/free-space observation, fixed-child inspection/creation, and first-set database/envelope/manifest-last create-new publication plus fresh verification are implemented; native picker implementation, independent first complete-set and exact-layout verification, every set-2 artifact, first and second complete sets, a two-set Layer D success owner, lifecycle composition and normal product reachability, operational recovery sets, restore, retention/deletion, and migration execution remain unimplemented. Focused manifest-publication tests cover the fixed filename, exact 98-byte partial-write loop, create-new conflict, flush/explicit close/fresh reopen order, strict parser and structural validator, short/trailing/magic/version/identifier/length/trusted-byte rejection, stable identity/path and parent-volume facts, ownership-bearing partial failure, redaction, and no cleanup or completion claim.
